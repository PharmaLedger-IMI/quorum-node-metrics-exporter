...
class Config:
    ...
    def load(self, config_object) -> bool:
        ...
        if not self._namespace:
            logging.error("'namespace' is not set in config. E.g. 'namespace': 'quorum'")
            return False

        self._deployment = config_object.get('deployment')
        if not self._deployment:
            logging.error("'deployment' is not set in config. E.g. 'deployment': 'quorum'")
            return False
        ...
        for each in peers:
            ...
            if enode is None or company_name is None or address is None or port is None:
                logging.error("'enode', 'company-name', 'enodeAddress', 'enodeAddressPort' must be set at a peer object.")
                return False
