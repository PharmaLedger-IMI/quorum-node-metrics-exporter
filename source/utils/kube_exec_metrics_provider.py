...
class KubeExecMetricsProvider(IMetricsProvider):
    ...
    def create_current_metrics(self, instance: str, instance_name: str, pod_name: str):
        ...
        for each_config_peer in self._config.peers.values():
            connection_successful = self._kube_exec_check_connectivity(pod_name=pod_name, peer=each_config_peer)
            metrics.add_metric(
                [instance, instance_name, each_config_peer.enode,
    ...
    def process(self):
        ...
        if pod_name is not None and pod_ip is not None:
            ...
            logging.warning("%s >> Cannot create metrics as pod_name or pod_ip is None - pod_name=%s, pod_ip=%s", type(self).__name__, pod_name, pod_ip)
